...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "newyorkgoldc " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
void ThreadOpenAddedConnections(void* parg)
{
    ...
    // Make this thread recognisable as the connection opening thread
    RenameThread("newyorkgoldc-opencon");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. newyorkgoldc is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    RenameThread("newyorkgoldc-start");
