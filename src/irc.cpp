...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #newyorkcTEST9\r");
            Send(hSocket, "WHO #newyorkcTEST9\r");
        } else {
            // randomly join #newyorkc00-#newyorkc99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = GetRandInt(50);

            Send(hSocket, strprintf("JOIN #newyorkc%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #newyorkc%02d\r", channel_number).c_str());
        }
