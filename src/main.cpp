...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // newyorkgoldc: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "newyorkgoldc Signed Message:\n";
...
if (nHeight < 20) {
        nSubsidy = 1000000 * COIN;
                ...
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 1111);
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 1799);
                nSubsidy = (1 + rand5) * COIN;
        } 
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 1499);
                nSubsidy = (1 + rand5) * COIN;
        } 
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 4999);
                nSubsidy = (1 + rand5) * COIN;
        } 
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 2999);
                nSubsidy = (1 + rand5) * COIN;
        } 
                ...
                cseed = cseed_str.c_str();
                seed = hex2long(cseed);
                rand5 = generateMTRandom(seed, 1299);
                nSubsidy = (1 + rand5) * COIN;
        } 
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand5 = generateMTRandom(seed, 49999);
        nSubsidy = (1 + rand5) * COIN;
    }
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand3 = generateMTRandom(seed, 6499);
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand2 = generateMTRandom(seed, 3299);
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 1299);
...
static const int64 nTargetTimespan = 60 * 5; // every 5 minutes
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal newyorkgoldc miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "newyorkgoldc", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "New York Gold - Love!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 88 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("555184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521738855;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1074923;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // newyorkgoldcd: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
